...
Currently configured for **CopilotKit Built-in Agent (React) + React** — the 21 routes of
...
```bash
(no separate agent server — the built-in agent runs inside Next)# http://localhost:3000
...
Videos land in `videos/` as `BUILTIN-react-<NN>-<name>.webm`, 1920×1080, ~25fps
...
every demo at `<route>/demo-chat`. This app tracks 23 doc routes; the 2 that carry no
