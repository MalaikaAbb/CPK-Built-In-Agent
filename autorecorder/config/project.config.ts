...
/**
 * NOTE — the one adaptation where `framework` and `docBaseUrl` legitimately
 * disagree, and `npm run doctor` warns about it.
 *
 * Every other integration lives under a doc slug (`docs.copilotkit.ai/agno/...`),
 * so the doctor checks that `docBaseUrl` contains `framework`. CopilotKit's
 * built-in agent has no slug: its pages are the documentation root itself —
 * `/quickstart`, `/prebuilt-components`, `/inspector`. All 21 doc URLs this
 * produces were confirmed to resolve 200.
 *
 * `framework` cannot be left empty (the doctor rejects empty required fields),
 * so it carries the nearest thing to a slug this integration has. The warning
 * is therefore expected here and must not be "fixed" by bending `docBaseUrl`,
 * which would break every doc URL. Reported as a `core/` gap: the slug-in-URL
 * assumption does not hold for the frameworkless case.
 */
export const PROJECT: ProjectConfig = {
  framework: "quickstart",
  frameworkLabel: "CopilotKit Built-in Agent (React)",
  videoPrefix: "BUILTIN-react",

  docBaseUrl: "https://docs.copilotkit.ai",
  ...
  backendStartCmd: "(no separate agent server — the built-in agent runs inside Next)",
