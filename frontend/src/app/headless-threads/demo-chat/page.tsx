 ...
 *
 * Only the first is available to us. `useThreads().startNewThread()` deselects
 * the list row and nothing else — it never touches the chat's `threadId`. The
 * second is `CopilotChatConfigurationValue.startNewThread`, unavailable here on
 ...
 * computed with `useMemo` at mount. Clearing the prop therefore returns to the
 * SAME id every time — the button appears to do nothing. Bumping a React `key`
 * forces the remount that re-runs that memo, which the Lifecycle page documents
 * exactly (as a footgun): "auto-minted ids are stable across re-renders, but
 * re-mint on remount". Here that is the intended effect.
 *
