 ...
 *
 * The three subscriber callbacks are the ones the Connect AG-UI agents page
 * shows — `onTextMessageContentEvent`, `onToolCallEndEvent`, `onStateChanged` —
 * plus the run lifecycle, so the start and end of a run are visible as
 * boundaries rather than inferred from the text stopping.
 *
...
export default function Page() {
  ...
  // Read inside the subscriber without re-subscribing when it toggles. Synced in
  ...
  useEffect(() => {
    ...
    const subscription = agent.subscribe({
      ...
      // Not every callback flattens its payload: some expose convenience fields,
      // others hand back only the raw `event`.
      ...
      onStateChanged({ agent }) {
        push("STATE_CHANGED", truncate(agent.state));
      },
    });
      ...
      <div className="grid h-full grid-cols-1 lg:grid-cols-2">
        ...
        <div className="min-h-0">
          <CopilotChat
            ...
            labels={{
              welcomeMessageText:
                "Anything you send here shows up as events on the left. Ask for the weather to see a tool call.",
            }}
          />
        </div>
      </div>
